    ...
    <div class="min-h-[calc(100vh-64px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <div class="max-w-md w-full">
                ...
                <div class="p-8">
                    <div class="text-center mb-8">
                        <div class="inline-flex items-center justify-center w-16 h-16 bg-green-50 text-green-600 rounded-full mb-4">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21v-2a4 4 0 00-4-4H9a4 4 0 00-4 4v2 m8-10a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                        </div>
                    ...
                    <div class="bg-gray-50 rounded-2xl p-6 mb-8 border border-gray-100 space-y-4">
                        ...
                        <div class="flex justify-between items-end">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Invited By</p>
                                <p class="text-sm font-semibold text-gray-700">{{ $invitation->inviter->name }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">For Account</p>
                                <p class="text-sm font-semibold text-gray-700">{{ $invitation->email }}</p>
                            </div>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('invitations.process', $invitation->token) }}" class="space-y-3">
                        @csrf
            ...
            <p class="mt-8 text-center text-xs text-gray-400 px-4 leading-relaxed">
                By accepting, you will gain access to shared expenses, settlements, and member details for <strong>{{ $invitation->colocation->name }}</strong>.
            </p>
        </div>
    </div>
</x-app-layout>